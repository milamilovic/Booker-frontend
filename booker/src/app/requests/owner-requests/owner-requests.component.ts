...
export class OwnerRequestsComponent {
  ...
  ngOnInit(): void {
    ...
    this.service.getAllForOwner(loggedId).subscribe({
  ...
  search(where_input: HTMLInputElement, search_date: HTMLInputElement) {
    ...
    if(this.accName==='' && this.searchDate==='') {
      ...
      if (filters.length != 0) {
        ...
        this.service.searchAndFilterOwnerRequests(this.searchDate, this.accName, loggedId, filters).subscribe({
          next: (data: ReservationRequest[]) => {
            this.requests = data
          },
          error: (_) => {
        ...
        this.service.searchOwnerRequests(this.searchDate, this.accName, loggedId).subscribe({
          next: (data: ReservationRequest[]) => {
            this.requests = data
          },
          error: (_) => {
  ...
  filter() {
    ...
    if (filters.length != 0) {
      this.service.filterOwnerRequests(loggedId, filters).subscribe({
        next: (data: ReservationRequest[]) => {
          this.requests = data
        },
        error: (_) => {
